    ...
    <section id="testimonials" ref={ref} className="py-20 md:py-32 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <h2 className="text-3xl md:text-4xl font-bold mb-4 text-white">{t.testimonials.sectionTitle}</h2>
          <div className="h-1 w-20 bg-muted-foreground mx-auto mb-4"></div>
          <p className="text-muted-foreground text-lg">{t.testimonials.sectionSubtitle}</p>
        </motion.div>
            ...
            <Card className="p-8 md:p-10 shadow-lg bg-black/30">
              <CardContent className="min-h-[200px]">
                {testimonials.map((testimonial, index) => {
                  // t.testimonialContents-ээс орчуулга авах
                  const translatedContent = t.testimonialContents[index]?.content ?? testimonial.content
                  return (
                    <motion.div key={testimonial.id}
                      initial={{ opacity: 0, x: 20 }}
                      animate={{ opacity: activeIndex === index ? 1 : 0, x: activeIndex === index ? 0 : 20, position: activeIndex === index ? "relative" : "absolute" }}
                      transition={{ duration: 0.5 }}
                      className={`${activeIndex !== index ? "hidden" : ""}`}>
                      <p className="text-lg md:text-xl italic mb-6 text-gray-200/80">{translatedContent}</p>
                ...
                <div className="flex justify-between items-center mt-8">
                  ...
                  <div className="flex space-x-5">
                    <Button variant="outline" size="icon" onClick={prev} className="hover:shadow-[0_0_20px_#9f7aea] transition-colors bg-black hover:bg-black cursor-pointer">
                      <ChevronLeft className="h-5 w-5 text-white" />
                    </Button>
                    <Button variant="outline" size="icon" onClick={next} className="hover:shadow-[0_0_20px_#9f7aea] transition-colors bg-black hover:bg-black cursor-pointer">
